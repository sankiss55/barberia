<?php
session_start();
include_once('conexion base de datos.php');
include_once('acciones.php');

$conexion = new conexionbd();
$conexion->conexion();
$acciones = new acciones($conexion);
$acciones->traerapartados();
if(!isset($_SESSION['usuario'])){
    header('Location: Errornoencontrado.html');
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
<link rel="stylesheet" href="estilos/productosapartados.css">
    <title>Document</title>
</head>
<body>
    <div id="divtodo">
<header><span id="regresarrr">&larr;</span>
<img src="imagenes/416191878_1092540801934607_5143561307543456637_n.jpg" alt="logo" id="logo">

<h1>Tus Productos a partados</h1>

<script>
        document.getElementById("regresarrr").addEventListener("click", function(){
            window.location.href="Productos.php";
        });
    </script>
</header>
    <main>
       <!---   aqui empiza la seccion donde sevisualliza el producto y te da pocos detalle del producto en este daso traemos a nombre, venta_ cantida y ventaprecioc_u que hacen referencia a nombre del producto, el dinero total a pagar de los tantos productos que adquirio y por ultimo el precio unitario --->
    <section>
        
    <?php
        for($i=0;$i<$acciones->contador;$i++){
?>
       <!---aqui pon un div -->
    <img width="100" src="data:image/jpeg;base64,<?php echo base64_encode($acciones->imga_pro[$i]); ?>">
    <div class="divdelosproductos">
    <p><?php echo $acciones->nombre[$i]; ?></p>
    <p>precio unitario:<?php echo $acciones->ventaprecioc_u[$i]; ?></p>
    <p><?php echo $acciones->venta_cantida[$i]; ?></p>
    </div><!---aqui termina el div que agregamos-->
    <?php
}
?>
    </section>
    <!---  en esta parte viene un como mas detallado su compra que es el subtotal el costo de envio y el total estimado-->
    <aside>
        <button id="comprar">Completar compra </button>
        <p>subtotal(<?php echo $acciones->ventacantidad_product; ?> articulos):    <?php echo $acciones->subtotal; ?></p>
        <p>costo de envio: $40</p>
        <p>Total estimado:<?php echo "$",$acciones->subtotal+40; ?></p>
    </aside>
    </main>
    </div>
     <!--- es cuestion de comodar todo bien por que si existe 100000 productos apartados en la seccion se deben que acomodar de buena manera osea que el diseño debe que estar al 100  -->
     <div id="payment-container" class="payment-container">
        <span id="quitar">&times;</span>
        <h2>Selecciona tu método de pago</h2>
        <div class="payment-option">
            <div class="oxxo" onclick="seleccionarMetodo('OXXO Pay')">
                <img src="data:image/png;base64," alt="OXXO Pay">
                <p>OXXO Pay</p>
            </div>
            <div class="tarjeta" onclick="seleccionarMetodo('Tarjeta')">
                <img src="data:image/jpeg;base64," alt="Tarjeta">
                <p>Tarjeta</p>
            </div>
        </div>
    </div>
    <script>
            document.getElementById("comprar").addEventListener("click", function(){
var divtodo=document.getElementById("divtodo");
divtodo.style.opacity="0.5";
var payment_container=document.getElementById("payment-container");
payment_container.style.display="block";
                
            });
            document.getElementById("quitar").addEventListener("click", function(){
var divtodo=document.getElementById("divtodo");
divtodo.style.opacity="1";
var payment_container=document.getElementById("payment-container");
payment_container.style.display="none";
            });
            function seleccionarMetodo(metododopago){
                if(metododopago=='OXXO Pay'){
                    window.location.href="terminardatosdepago.php";
                }else if(metododopago=='Tarjeta'){
                    window.location.href="terminardatosdepagotarjeta.php";
                }
            }
        </script>
    </body>

</html>